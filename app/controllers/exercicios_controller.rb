class ExerciciosController < ApplicationController
  ...
  def index
    ...
    @exercicios = @exercicios.where(tipo_de_exercicio_id: params[:tipo_de_exercicio]) if params[:tipo_de_exercicio].present?
    
    options = {page: params[:page], per_page: 20}
    @exercicios = @exercicios.paginate(options)
  end
  ...
  private
    ...
    def exercicio_params
      params.require(:exercicio).permit(:nome, :tutorial, :tipo_de_exercicio_id)
    end
end
